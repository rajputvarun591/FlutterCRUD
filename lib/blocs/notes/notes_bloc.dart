...
class NotesBloc extends Bloc<NotesEvent, NotesState> {
  ...
  NotesBloc({@required this.repository})
      : assert(repository != null), super(InitialState());
  ...
  Stream<NotesState> _mapShowNotesToEvent(ShowNotes event, NotesState currentState) async* {
    try {
      ...
      yield NotesLoading();
      final List<Notes> list = await repository.getNotes(columnName: event.sortBy, order: event.orderBy, limit: 8, offSet: 0);
      yield NotesLoaded(notes: list, hasReachMAx: (list.length < 8), sortBy: event.sortBy, orderBy: event.orderBy);
    } catch (e) {
  ...
  Stream<NotesState> _mapAddNoteToEvent(AddNote event, NotesState currentState) async* {
    try {
      final response = await repository.addNote(notes: event.notes);
      if (response == 0) {
  ...
  Stream<NotesState> _mapUpdateNoteToEvent(UpdateNote event, NotesState currentState) async* {
    yield NotesLoading();
    try {
      final response = await repository.updateNote(notes: event.notes);
      if (response == 0) {
  ...
  Stream<NotesState> _mapDeleteNoteToEvent(DeleteNote event, NotesState currentState) async* {
    yield NotesLoading();
    try {
      final response = await repository.deleteNote(note: event.notes);
      if (response == 0) {
  ...
  Stream<NotesState> _mapUpdateFavoriteStatusToEvent(
    ...
    //yield NotesLoading();
    try {
      final response = await repository.updateFavoriteStatus(notes: event.notes);
      if (response == 0) {
        yield NotesFailure(message: "Not Updated");
      } else {
        if (currentState is NotesLoaded) {
  ...
  Stream<NotesState> _mapMoveToTrashToEvent(MoveToTrash event, NotesState currentState) async* {
    try {
      final response = await repository.updateDelAction(notes: event.note);
      if (response == 0) {
        yield NotesFailure(message: "Not Updated");
      } else {
        if (currentState is NotesLoaded) {
